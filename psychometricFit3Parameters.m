...
function output = psychometricFit3Parameters(x,y,subjNum,condition,save)
    %hold the previous figure
    figure;

    %psyFit = @(p,a) 0.5*(erfc((a-p(1))/(p(2)*sqrt(2))));
    %   y  =        0.5*(erfc(-(k   .*beta).*(x-alpha)./sqrt(2)));
    psyFit = @(p,a) 0.5*(erfc(-(p(3).*p(2)).*(a- p(1))./sqrt(2)));
    
    %Get an estimate of the parameters from the function and guess valuese
    pEst = nlinfit(x,y,psyFit,[0.5, 1, 1]);
    ...
    title(['Subject ', subjNum, ', Condition ', condition]);
    xlim([-1 1]);
    ylim([0 1]);
